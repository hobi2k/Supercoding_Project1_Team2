...
## 팀원(임시)
- **JaewonYooTime**: CTO, 회의록 작성, 코드 통합
...
2. **기초 통계**  
   1. 데이터 프로파일링:
   - 데이터가 가진 특성에 대해서 인사이트를 뽑는 목적
   - Rates: 데이터셋의 특성상 가장 인기있는 1000개이므로 Rates는 상향평준화, 즉 ... ~를 고려해야한다.
   - Votes: 오래된 영화는 명화이고 그래서... 누적된다... 그래서... ~를 고려해야한다.
   - Gross: 물가상승률을 반영하고 그러면 오를수밖에 없는 특성...그래서... ~를 고려해야한다.
   2. 시대별, 장르별 Rates, Votes, Gross 평균 시각화
   3. 장르 상관없이 Rates, Votes, Gross 평균 시각화
   4. **(추가)** 주요 사건, 도시화, 영화 제작 편수와 데이터의 상관관계 출력
